...
// Testlar to'liq superset ("all") profilida ishlaydi — eski va yangi to'plamlar birga
process.env.SITE_PROFILE = 'all';
// Element (CMS override) tizimini tekshirish uchun vaqtinchalik manifest fixture'i.
// Production'da manifest yo'q (TalabaHub kontenti to'liq bazadan keladi).
const MANIFEST_FIXTURE = path.join(TMP, 'cms-manifest.json');
fs.writeFileSync(MANIFEST_FIXTURE, JSON.stringify([
  { id: 'cms-0001', type: 'video', label: 'Hero video', context: 'hero', tag: 'video', original: { src: '/hero.mp4' } },
  { id: 'cms-0138', type: 'text', label: 'Intro', context: 'intro', tag: 'p', editableText: true, original: { text: 'Salom' } },
]));
process.env.MANIFEST_PATH = MANIFEST_FIXTURE;
...
test('public site seed ma\'lumot qaytaradi', async () => {
  ...
  // TalabaHub xizmatlari seed qilinadi
  assert.ok(Array.isArray(data.roommates) && data.roommates.length >= 3);
  assert.ok(Array.isArray(data.jobs) && data.jobs.length >= 4);
  assert.ok(Array.isArray(data.market) && data.market.length >= 4);
  assert.ok(Array.isArray(data.partners) && data.partners.length >= 4);
  // Pullik (featured) e'lonlar ro'yxat tepasida ko'rinadi
  assert.strictEqual(data.jobs[0].featured, 1);
  assert.strictEqual(data.partners[0].featured, 1);
});
...
test('CSRF token siz login rad etiladi', async () => {
  ...
  const res = await fetch(base + '/api/admin/login', {
    ...
    body: JSON.stringify({ email: 'staff@talabahub.uz', password: 'TestStaff123' }),
  });
...
test('staff login + noto\'g\'ri parol', async () => {
  ...
  const bad = await req('POST', '/api/admin/login', { email: 'staff@talabahub.uz', password: 'wrong' });
  assert.strictEqual(bad.status, 401);
  const ok = await req('POST', '/api/admin/login', { email: 'staff@talabahub.uz', password: 'TestStaff123' });
...
test('yangi xizmat to‘plami CRUD + featured bayrog‘i', async () => {
  // staff sifatida (oldingi testda login qilingan) yangi partner qo'shamiz
  const created = await req('POST', '/api/admin/partners', { name: 'Test Kafe', category: 'Kafe', discount: '-25%', featured: true });
  assert.strictEqual(created.status, 201);
  assert.strictEqual(created.data.item.featured, 1); // checkbox -> 1 ga normallashadi
  // featured DESC tartib: yangi featured yozuv ro'yxat tepasiga chiqadi
  const list = await req('GET', '/api/admin/partners');
  assert.strictEqual(list.status, 200);
  assert.strictEqual(list.data.items[0].featured, 1);
  // featured'ni o'chiramiz
  const upd = await req('PUT', `/api/admin/partners/${created.data.item.id}`, { name: 'Test Kafe', featured: 0 });
  assert.strictEqual(upd.status, 200);
  assert.strictEqual(upd.data.item.featured, 0);
  const del = await req('DELETE', `/api/admin/partners/${created.data.item.id}`);
  assert.strictEqual(del.status, 200);
});
...
test('editor RBAC — users/settings/video rad etiladi', async () => {
  ...
  await req('GET', '/api/admin/session');
  await req('POST', '/api/admin/login', { email: 'admin@talabahub.uz', password: 'TestEditor123' });
...
test('PII himoyasi: arizalar faqat staff uchun', async () => {
  ...
  cookies = {};
  await req('GET', '/api/admin/session');
  await req('POST', '/api/admin/login', { email: 'staff@talabahub.uz', password: 'TestStaff123' });
...
test('sayt profili: barcha to‘plamlar birlashgan rejimda ochiq', async () => {
  ...
  // TalabaHub xizmat to'plamlari "all" da faol
  assert.ok(has('jobs') && has('market') && has('partners') && has('roommates'));
  // "hub" profili faqat xizmatlardan iborat (universitet bo'limlarisiz)
  assert.ok(PROFILES.hub.collections.includes('jobs') && PROFILES.hub.collections.includes('partners'));
  assert.ok(!PROFILES.hub.collections.includes('news'));
