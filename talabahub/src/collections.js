...
/** Checkbox/0-1 qiymatni 0 yoki 1 ga keltiradi (pullik "featured" bayrog'i uchun). */
const flag = (v) => (v === true || v === 1 || v === '1' || v === 'on' || v === 'true') ? 1 : 0;

/** Pullik joylashtirilgan (featured) e'lonlar tepada; keyin qo'lda tartib. */
const FEATURED_ORDER = 'featured DESC, sort ASC, id ASC';
...
const ALL_COLLECTIONS = {
  ...
  residences: {
    ...
    columns: ['name', 'city', 'image', 'summary', 'description', 'amenities', 'price', 'link', 'featured', 'sort'],
    required: 'name',
    sanitize: (b) => ({
  ...
  roommates: {
    columns: ['title', 'gender', 'faculty', 'course', 'area', 'budget', 'about', 'contact', 'image', 'sort'],
    required: 'title',
    sanitize: (b) => ({
      title: sanitizeString(b.title, 300), gender: sanitizeString(b.gender, 30),
      faculty: sanitizeString(b.faculty, 200), course: sanitizeString(b.course, 40),
      area: sanitizeString(b.area, 200), budget: sanitizeString(b.budget, 120),
      about: sanitizeString(b.about, 4000), contact: sanitizeString(b.contact, 200),
  ...
  jobs: {
    order: FEATURED_ORDER,
    columns: ['title', 'company', 'jobtype', 'pay', 'area', 'description', 'contact', 'link', 'featured', 'sort'],
    required: 'title',
    sanitize: (b) => ({
      title: sanitizeString(b.title, 300), company: sanitizeString(b.company, 200),
      jobtype: sanitizeString(b.jobtype, 60), pay: sanitizeString(b.pay, 120),
      area: sanitizeString(b.area, 200), description: sanitizeString(b.description, 8000),
      contact: sanitizeString(b.contact, 200), link: sanitizeUrl(b.link),
      featured: flag(b.featured),
      sort: Number.isFinite(Number(b.sort)) ? Number(b.sort) : 0,
    }),
  },
  market: {
    order: FEATURED_ORDER,
    columns: ['title', 'category', 'price', 'condition', 'area', 'description', 'contact', 'image', 'featured', 'sort'],
    required: 'title',
    sanitize: (b) => ({
      title: sanitizeString(b.title, 300), category: sanitizeString(b.category, 120),
      price: sanitizeString(b.price, 120), condition: sanitizeString(b.condition, 60),
      area: sanitizeString(b.area, 200), description: sanitizeString(b.description, 8000),
      contact: sanitizeString(b.contact, 200), image: sanitizeUrl(b.image),
      featured: flag(b.featured),
      sort: Number.isFinite(Number(b.sort)) ? Number(b.sort) : 0,
    }),
  },
  partners: {
    order: FEATURED_ORDER,
    columns: ['name', 'category', 'discount', 'area', 'description', 'image', 'link', 'featured', 'sort'],
    required: 'name',
    sanitize: (b) => ({
      name: sanitizeString(b.name, 300), category: sanitizeString(b.category, 120),
      discount: sanitizeString(b.discount, 120), area: sanitizeString(b.area, 200),
      description: sanitizeString(b.description, 8000), image: sanitizeUrl(b.image),
      link: sanitizeUrl(b.link), featured: flag(b.featured),
      sort: Number.isFinite(Number(b.sort)) ? Number(b.sort) : 0,
    }),
  },
  gallery: {
